...
int hostMatch(long *comparisons)
{
    ...
    long comparisons_tmp = -1;
    int found = textLength + 1;

    #pragma omp parallel for reduction(+:comparisons_tmp) schedule(static) num_threads(4) default(none) shared(found, comparisons) private(j, k) firstprivate(lastI, textData, patternData, patternLength)
    for(i = 0; i<= lastI; i++)
    {
         if(i < found)
         {
	    while(j < patternLength){
	        comparisons_tmp++;
                if(textData[k] == patternData[j] && j < patternLength)
                {
                    k++; j++; 	
                }
		else{
