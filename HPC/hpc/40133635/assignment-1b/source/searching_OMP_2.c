...
{
	int i,j,lastI;
	int position = -1;  // set to -1 to initialse pattern to not be found
	int volatile flushflag =0;
	long comparisons_tmp = 0;
	lastI = textLength-patternLength;
	*comparisons =0;
        #pragma omp parallel num_threads(numThreads) private(i,j) shared(position,flushflag) reduction(+: comparisons_tmp)	
	...
	for(i=0; i<=lastI; i++)
	{
	    ...
	    {
	        j=0;
	        while(j<patternLength && textData[i+j] == patternData[j] && position==-1)
	        {
		    j++;
		    comparisons_tmp++;
		}
	...
	*comparisons = comparisons_tmp;
	...
	return position; // returns -1 if pattern not found 
